...
def analyze_page(url):
    ...
    # Broken links (HEAD request only)
    broken_links = []
    ...
    # 2️⃣ IFRAME detection
    has_iframe = soup.find("iframe") is not None

    # 3️⃣ FAVICON detection
...
@timer
def check_robots_for_sitemap(base_url):
    ""Check robots.txt for sitemap declaration and return sitemap URL if found""
...
def analyze_single_url():
    ...
    if form.validate_on_submit():
        try:
            ...
            # --- START FIX: Fetch HTML content once and make it available ---
            html_content = fetch_html(url)
            if not html_content:
                flash("Failed to fetch HTML content for analysis.", "danger")
                return redirect(url_for("analyze_single_url"))
            # --- END FIX ---
            ...
            analytics_results = detect_analytics(html_content)
            ...
            #analytics_services = check_analytics(url)
            ...
            metrics = {
                ...
                "Total Images": [seo_metrics.get("Total Images", "N/A")],
